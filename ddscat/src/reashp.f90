...
! 19.05.11 (BTD): added code to check for non-duplication
! end history

! Copyright (C) 1993,1995,1996,1998,2001,2004,2007,2008,2009,2010,2012
!               2014,2019 B.T. Draine and P.J. Flatau
...
! 19.05.11 (BTD) add code for sanity check for non-duplication of sites
      DO J=2,NAT
         DO K=1,J-1
            IF(IXYZ(J,1).EQ.IXYZ(K,1).AND.  &
               IXYZ(J,2).EQ.IXYZ(K,2).AND.  &
               IXYZ(J,3).EQ.IXYZ(K,3))THEN
               WRITE(CMSGNM,FMT='(A)')'error in input shape file '
               CALL WRIMSG('REASHP',CMSGNM)
               WRITE(CMSGNM,FMT='(A,I8,A,I8)')           &
                  'duplication for dipoles K=',K,' and J=',J
               CALL WRIMSG('REASHP',CMSGNM)
               WRITE(CMSGNM,FMT='(A,I8,A,I6,A,I6,A,I6)')'K=',K,   &
                  ' IXYZ(K,1)=',IXYZ(K,1),' IXYZ(K,2)=',IXYZ(K,2), &
                  ' IXYZ(K,3)=',IXYZ(K,3)
               CALL WRIMSG('REASHP',CMSGNM)
               WRITE(CMSGNM,FMT='(A,I8,A,I6,A,I6,A,I6)')'J=',J,   &
                  ' IXYZ(J,1)=',IXYZ(J,1),' IXYZ(J,2)=',IXYZ(J,2), &
                  ' IXYZ(J,3)=',IXYZ(J,3)
               CALL WRIMSG('REASHP',CMSGNM)
