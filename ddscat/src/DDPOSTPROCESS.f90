...
!
!    ILINE    = 0 to skip evaluation of any field at points on lines
!               1 to evaluate E and P at points along 1 or more lines
!                 (if DDSCAT was run with NRFLD=1)
!                 and also B and S=Poynting vector
!                 (if DDSCAT was run with NRFLD=2)
...
! 17.10.12 (BTD) v3 modified to also write out polarizations P
!                   along one or more lines
!                   polarization P is written to output file
!                   ddpostprocess_P.out
! 19.05.17 (BTD) v4 modified to use subroutine VERSION to identify
!                   distribution vintage
! 19.08.25 (BTD) v5 fixed problem with location of GOTO 3000
! end history
...
!=======================================================================
      ...
      WRITE(IDVOUT,FMT='(3A)')'>DDPOSTPROCESS: from ',CSTAMP,' distribution'
...
! calculate normalized Poynting vector at all points
      ...
      IF(ILINE>0)THEN
         ...
         OPEN(UNIT=8,FILE='ddpostprocess_P.out')
         WRITE(IDVOUT,FMT='(A)')'>DDPOSTPROCESS: begin computing E along tracks'
...
! check that track is entirely within computational volume
         ...
         IF(XA>=XMIN.AND.XB<=XMAX.AND. &
            ...
            DO JDEV=7,8
               WRITE(JDEV,FMT='(1PE10.4,A)')                    &
                  AEFF,' = a_eff (radius of equal-volume sphere)'
               WRITE(JDEV,FMT='(1PE10.4,A)')DPHYS,' = d = dipole spacing'
               WRITE(JDEV,FMT='(I10,A)')                         &
                  NAT0,' = N = number of dipoles in target or TUC'
               WRITE(JDEV,FMT='(3I8,A)')                                &
                  NX,NY,NZ,' = NX,NY,NZ = extent of computational volume'
               WRITE(JDEV,FMT='(1PE10.4,A)')WAVE,' = wavelength in vacuo'
               WRITE(JDEV,FMT='(F10.5,A)')                       &
                  NAMBIENT,' = refractive index of ambient medium'
               WRITE(JDEV,FMT='(3F10.5,A)')AK_TF,' = k_{inc,TF} * d'
               WRITE(JDEV,FMT='(5(2F10.5,A,/),2F10.5,A)')                   &
                  CXE0_TF(1),                                               &
...
!***
         ...
         ELSE
            ...
            WRITE(7,FMT='(3X,A,7X,A,7X,A,6X,A,11(3X,A),2X,A)')     &
               ...
               '-- <(Sx,Sy,Sz)>/|<S_inc>| --'                      !
         ENDIF
         WRITE(8,FMT='(3X,A,7X,A,7X,A,6X,A,3X,A,3X,A,3X,A,3X,A,3X,A)') &
               'x_TF','y_TF','z_TF','Re(P_x)','Im(P_x)','Re(P_y)',     &
               'Im(P_y)','Re(P_z)','Im(P_z)'
         DO JA=0,NAB
