...
a DDSCAT run found in a ddscat.par file are stored in pyddscat in a
...
>>> s = Settings()
>>> print s
<pyddscat.core.Settings object at 0x10a544550>
...
fields, and the values they can take is found in the `pyddscat API`_ under 
...
Polarization
^^^^^^^^^^^^
pyddscat includes named polarizations that can be used to simplify polarization
specification
...
Ranges
^^^^^^
pyddscat defines classes for managing ranges used to specify things like
...
is done by specifying a range for ``aeff``. In pyddscat, targets have a defined
