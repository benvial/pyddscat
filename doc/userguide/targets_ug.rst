...
Making target creation simple is one of the central goals of pyddscat. Target class definitions
reside in the module :mod:`targets`. One important distinction between the way
that targets in pyddscat behave vs. DDSCAT is that in pyddscat targets always
...
>>> t = targets.Target()
>>> print(t)
<pyddscat.targets.Target object at 0x7f8a28d0d4d0>
>>> t.directive
'RCTGLPRSM'
...
>>> print(t)
<pyddscat.targets.Target object at 0x13fa2ff50>
...
and a_eff. The dipole spacing is inferred from these two. pyddscat works the
...
reads the geometry from a ``shape.dat`` file supplied by the user. pyddscat
...
Custom targets can be viewed using the ``show`` method::
...
interface to a unique target geometry. For instance pyddscat includes a custom
...
repeat that TUC on the desired grid.
    ...
    >>> helix = pyddscat.targets.Helix(0.2, 0.1, 0.05, 0.02)
    Generating Helix...
...
>>> print(t)
<pyddscat.targets.Iso_FROM_FILE object at 0x11df95b90>>>>
>>> t.N
13290
>>> t.aeff
0.29112538896800289
...
The Future
==========
Future versions of pyddscat aim to implement simple geometric transformations
