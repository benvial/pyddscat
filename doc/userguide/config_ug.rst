...
*********************
.. currentmodule:: pyddscat

We have seen earlier that when you create new settings or targets using
``Settings()`` or ``Target()`` with no arguments pyddscat generates them
based on default files. This chapter will discuss the locations and contents
of those default. There are two distinct types of defaults used by pyddscat:

Profiles
    These are plain python scripts which tailor pyddscat's output for different
...
#. In the configuration folder in the user's home directory ``~/.pyddscat``
#. In the subfolder ``profiles/`` of the directory containing ``pyddscat.core``.
   Generally this will be something like ``site-packages/pyddscat/profiles
...
.. literalinclude:: ../../../pyddscat/profiles/default.py
...
.. literalinclude:: ../../../pyddscat/profiles/example_mpi.py
