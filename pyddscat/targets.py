...
class Target(object):
    ...
    * fromfile(): A class method to generate a working object from a ddscat.par file
    ...
    def __init__(
        self, directive=None, sh_param=None, material=None, aeff=None, folder=None
    ):
    ...
    def save_str(self):
        ...
        for mat in self.material:
            out += f"'{utils.resolve_mat_file(mat)}" + "'\n"
    ...
    def VTRconvert(self, outfile=None, verbose=False):
    ...
    def fromfile(cls, fname):
        ...
        if directive in globals():
            ...
            try:
                ...
                warnings.warn(
                    f"Class {subclass} has no fromfile method. Returning a generic Target"
                )
    ...
    def make_periodic(self, period):
        ...
        if new_cls is None:
            raise TypeError(
                f"No corresponding periodic class for {str(self.__class__)}."
            )
    ...
    def show(self, *args, **kwargs):
        ...
        # filename = kwargs.pop["filename"] if "filename" in kwargs else "output.pvd"
        filename = os.path.join(self.folder, filename)
        reader = pyvista.get_reader(filename)
        grid = reader.read()[0]  # MultiBlock mesh with only 1 block
        sphere = pyvista.Sphere(theta_resolution=10, phi_resolution=10)
        spheres = grid.glyph(geom=sphere, orient=False)  # , tolerance=0.1)
...
class RCTGLPRSM(Target_Builtin):
    ...
    def __init__(self, phys_shape, d=None, material=None, folder=None):
        Target_Builtin.__init__(
            self, "RCTGLPRSM", d=d, material=material, folder=folder
        )
    ...
    def _calc_N(sh_param):
        ...
        """
        return np.array(sh_param[:3]).prod()
...
class CYLNDRCAP(Target_Builtin):
    ...
    def __init__(self, length, radius, d=None, material=None, folder=None):
        Target_Builtin.__init__(
            self, "CYLNDRCAP", d=d, material=material, folder=folder
        )
        self.length = length
        self.radius = radius
...
class ELLIPSOID(Target_Builtin):
    ...
    def __init__(self, semiaxes, d=None, material=None, folder=None):
        Target_Builtin.__init__(
            self, "ELLIPSOID", d=d, material=material, folder=folder
        )
...
class CYLINDER(Target_Builtin):
    ...
    def __init__(self, length, radius, orient, d=None, material=None, folder=None):
        ...
        Target_Builtin.__init__(
            self, "CYLINDER1", d=d, material=material, folder=folder
        )
...
class FROM_FILE(Target):
    ...
    def __init__(self, grid=None, d=None, material=None, folder=None):
        ...
        self.description = "FROM_FILE"
        self.fname = "shape.dat"

        self.d = d or default_d

        self.grid = grid if grid is not None else np.zeros((0, 0, 0, 3), dtype=int)
    ...
    def N(self):
        ...
        if len(self.grid.shape) == 4:
            flat = self.grid.sum(3).astype(bool)
        else:
            flat = self.grid.astype(bool)
    ...
    def _grid2table(self):
        ...
        grid = self.grid.sum(3) if len(self.grid.shape) == 4 else self.grid
    ...
    @classmethod
    def fromfunction(
        cls, func, pt1, pt2, origin=None, d=None, material=None, folder=None, **kwargs
    ):
        """
        ...
        origin = np.array(origin) if origin else 0.5 * (pt1 + pt2)
        val = func(*origin)
    ...
    def _mayavi_show(self, *args, **kwargs):
        """
        ...
        geometry_filter = mlab.pipeline.user_defined(
            pts, filter="GeometryFilter", name="VisibleVolume"
        )
...
class Ellipsoid_FF(Iso_FROM_FILE):
    ...
    def __init__(self, semiaxes, d=None, material=None, folder=None):
        ...
        self.description = "Ellipsoid_FF (%f, %f, %f, %f)" % (
            tuple(self.phys_shape) + (self.d,)
        )
...
class Helix(Iso_FROM_FILE):
    ...
    def __init__(
        self, height, pitch, major_r, minor_r, d=None, material=None, folder=None
    ):
        Iso_FROM_FILE.__init__(self, d=d, material=material, folder=folder)
    ...
    def _build_helix(self):
        ...
        self.description = "FROM_FILE_Helix (%f, %f, %f, %f, %f)" % (
            self.height,
            self.pitch,
            self.major_r,
            self.minor_r,
        ...
        # the sweep path
        t = np.linspace(0, 1, int(self.height / abs(self.pitch)) * 360)
        ...
        def sphere_check(i, j, k):
            ...
            return 1 if np.any(d <= p_minor_r) else 0
...
class SpheresHelix(Iso_FROM_FILE):
    ...
    def __init__(
        self, height, pitch, major_r, minor_r, n_sphere, d=None, build=True, **kwargs
    ):
        if d is None:
            d = default_d
        d_shape = np.int16(
            np.asarray(
                [height + 2 * minor_r, 2 * (major_r + minor_r), 2 * (major_r + minor_r)]
            )
            / d
        )
        d_shape += 1
...
class Conical_Helix(Iso_FROM_FILE):
    ...
    def __init__(
        self, height, pitch1, pitch2, major_r, minor_r, d=None, build=True, **kwargs
    ):
        if d is None:
            d = default_d
        d_shape = np.int16(
        ...
        if build:
            ...
            self.description = "FROM_FILE_Helix (%f, %f, %f, %f, %f, %f)" % (
                self.height,
                self.pitch1,
                self.pitch2,
                self.major_r,
    ...
    def build_helix(self):
        ...
        # the helix dimensions in pixels
        p_height = (
            self.height * (1 - (self.pitch2 - self.pitch1) / self.pitch2)
        ) / self.d
...
class Polygon(Iso_FROM_FILE):
    ...
    def __init__(
        self, polygon, thickness, bbox=None, d=None, material=None, folder=None
    ):
        Iso_FROM_FILE.__init__(self, d=d, material=material, folder=folder)
        ...
        self.origin = np.array(tuple(poly_centre) + (thickness / 2.0,))
...
class CYLNDRPBC(CYLINDER, Periodic):
    ...
    def __init__(
        self, length, radius, orient, period, d=None, material=None, folder=None
    ):
        CYLINDER.__init__(
            self, length, radius, orient, d=d, material=material, folder=folder
        )
    ...
    def sh_param(self):
        ...
        return (CYLINDER.sh_param.fget(self)) + (
            self.period[0] / self.d,
            self.period[1] / self.d,
        )
