...
def resolve_mat_file(material):
    ...
    if path.dirname(material) != "":
        ...
        return path.normpath(
            path.join(path.expanduser(config["mat_library"]), material)
        )
...
def resolve_profile(fname):
    ...
    for path in [
        "./",
        os.path.expanduser("~/.pyddscat/"),
        os.path.join(pkg_path, "profiles"),
    ]:
...
def make_profile():
    ...
    home_path = os.path.expanduser("~/.pyddscat/")
    try:
...
def compress_files(folder=None, recurse=False):
    ...
    for ext in ["avg", "sca", "fml", "pol*", "E*"]:
        if len(glob.glob1(folder, f"*.{ext}")):
