...
class Table(dict):
    ...
    def plot(
        ...
        **kwargs,
        ...
        ylbl = "".join(f"{f}, " for f in y_fields[:])
        ...
        for f in y_fields:
            y = self[f]
            l_text = f if label is None else label
            try:
                if l_text[0] == "_":
                    l_text = l_text[1:]
            except IndexError:
            ...
            maxy = y.max() if normalize else 1.0
            if smooth is False:
...
class AVGSummaryTable(Table):
    ...
    def __init__(
        self, fname=None, folder=None, npol=None, ncomps=None, zfile=None, **kwargs
    ):
        if fname is None:
            fname = "w000r000.avg"
        self.fname = fname
    ...
    def _refresh_1pol(self, f):
        ...
        hdr = "".join(f.readline() for _ in range(31 + self.ncomps))
        self.header = hdr
        hdr = hdr.splitlines()
    ...
    def _refresh_2pol(self, f):
        ...
        """
        hdr = "".join(f.readline() for _ in range(36 + self.ncomps))
        self.header = hdr
        hdr = hdr.splitlines()
        ...
        self["Epol"] = np.array([utils.str2complexV(l1), utils.str2complexV(l2)])
...
class SCASummaryTable(Table):
    ...
    def _refresh_1pol(self, f):
        ...
        hdr = "".join(f.readline() for _ in range(43))
        self.header = hdr
        hdr = hdr.splitlines()
...
class QTable2(ResultTable):
    ...
    def __init__(self, fname=None, folder=None, **kwargs):
        ...
        hdr_lines = _find_header_len(fname, folder, "aeff")
        ResultTable.__init__(
            self, fname, hdr_lines, [10, 11, 12, 12, 12], folder=folder, **kwargs
        )
        self.x_field = "wave"
        self.y_fields = ["Q_pha"]
    ...
    def show(self, show_now=True, *args, **kwargs):
        ...
        if "mask_points" in kwargs:
            ...
            utils.logger.warning(
                """Large number of datapoints in target. 
                Plotting only a subset. Specify mask_points=None or an integer 
                to force skipping value"""
            )
...
class EnTable(dict):
    ...
    def _load(self, fid):
        ...
        E_inc = self.E_inc
        self.E_inc = E_inc[::2] + 1j * E_inc[1::2]
    ...
    def show(self, field="Etot2", phase=0.0, mask_points=None, show_now=True):
        ...
        if f.dtype in [np.dtype("complex64"), np.dtype("complex128")]:
...
class ResultCollection(OrderedDict):
    ...
    def plot(self, fields=None, label=None, normalize=False, lw=2, **kwargs):
        ...
        for key, table in self.items():
            ...
            l_txt = key if label is None else label
        ...
        flds = table.y_fields if fields is None else fields
        ylbl = "".join(f"{f}, " for f in flds)
        ylbl = ylbl[:-2]
        ...
        for L in list(self.keys()):
            if L.endswith(("LCP", "_cL")):
                ...
                if R in list(self.keys()):
                    ...
                    utils.logger.trace(f"No complement found for {L}")
...
class FolderCollection(ResultCollection):
    def __init__(self, r_type=None, path=None, recurse=True, fields=None):
        ...
        folders = []
        if recurse:
            folders.extend(root for root, dirs, files in os.walk(path))
        else:
...
class FileCollection(ResultCollection):
    ...
    def _avgsum_dichroism(self, fields=None):
        ...
        table = [v.dichroism() for k, v in self.items()]
        CD = Table()
    ...
    def split(self):
        ...
        C = ResultCollection()
        C[f"_{utils.pol2str(P1.Epol)}"] = P1
        C[f"_{utils.pol2str(P2.Epol)}"] = P2
...
class SCAHyperSpace:
    ...
    def __init__(self):
        self.zfile = "all_sca.zip" if len(glob.glob("*.sca")) == 0 else None
        self.ProcessSCASpace()
        self.shape = (
            len(self.w_range),
            len(self.r_range),
            len(self.beta_range),
            len(self.theta_range),
            len(self.phi_range),
            len(self.Epol_range),
            2 + len(self.Epol),
        )
    ...
    def refresh(self):
        ...
        Beta = dict(list(zip(self.beta_range, list(range(len(self.beta_range))))))
        Theta = dict(list(zip(self.theta_range, list(range(len(self.theta_range))))))
        ...
        for f in flist:
            ...
            if len(self.Epol) == 1:
                ...
                dat = np.array(
                    (sca["Q_ext"], sca["Q_abs"], sca["Q_sca"], sca["Q_pol"])
                ).transpose()
    ...
    def dichroism(self):
        ...
        return (
            _dichroism_calculator(self.data[..., 0, :], self.data[..., 1, :])
            if a == 0
            else _dichroism_calculator(self.data[..., 1, :], self.data[..., 0, :])
        )
...
def plot_dichroism_from_avg(fields=None, lw=None, normalize=False, **kwargs):
    ...
    ylbl = "".join(f"{f}, " for f in fields[1:])
    ylbl = ylbl[:-2]
